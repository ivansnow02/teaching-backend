...
func EncEmail(email string) (string, error) {
	data, err := codec.EcbEncrypt([]byte(emailAesKey), []byte(email))
	if err != nil {
		return "", err
	}
...
func DecEmail(email string) (string, error) {
	originalData, err := base64.StdEncoding.DecodeString(email)
	if err != nil {
		return "", err
	}
